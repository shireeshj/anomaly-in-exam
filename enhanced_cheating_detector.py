...
This script implements a hybrid approach combining Isolation Forest with Random Forest
to achieve high accuracy in detecting exam cheating.
"""
...
for user_id in labeled_data['elitmus_id']:
    ...
    if len(user_questions) > 0:
        ...
        features['time_std'] = times.std()
        features['time_consistency'] = features['time_std'] / features['avg_time'] if features['avg_time'] > 0 else 0
        features['time_min'] = times.min()
        features['time_max'] = times.max()
        features['time_range'] = features['time_max'] - features['time_min'] if len(times) > 0 else 0
        
        # Calculate time for correct vs incorrect answers
        ...
        if 'avg_time' in user_questions:
            fast_correct = ((user_questions['correct'] == True) & 
                           (user_questions['seconds_taken'] < user_questions['avg_time'] * 0.3)).sum()
        ...
        if features['time_consistency'] < 0.1 and features['total_questions'] > 5:
        ...
        features['time_min'] = 0
        features['time_max'] = 0
        features['time_range'] = 0
        features['avg_time_correct'] = 0
        features['avg_time_incorrect'] = 0
        features['time_ratio_correct_incorrect'] = 0
        features['suspicious_pattern_score'] = 0
    ...
    if len(user_activities_data) > 0:
        ...
        if 'created_at' in user_activities_data.columns:
            try:
                ...
                if len(time_diffs) > 0:
                    ...
                    features['time_between_std'] = time_diffs.std()
                    
                    # Calculate suspicious timing score
                    suspicious_timing = 0
                    
                    # Check for unusually consistent timing between activities
                    if features['time_between_std'] < 0.5 and len(time_diffs) > 10:
        ...
        features['navigation_frequency'] = 0
        features['back_navigation_ratio'] = 0
        features['avg_time_between_activities'] = 0
        features['time_between_std'] = 0
        features['suspicious_timing_score'] = 0
...
X_train_with_scores = np.column_stack((X_train_scaled, train_scores))
X_test_with_scores = np.column_stack((X_test_scaled, test_scores))
...
# Add overall title
plt.suptitle('Enhanced Cheating Detection Results', fontsize=24, fontweight='bold', y=0.98)
...
# Save visualization
save_path = os.path.join(output_dir, "enhanced_cheating_detection.png")
...
print("\nEnhanced Cheating Detection completed successfully!")
